...
function removeDeadPlayers() {
    _.without(players, _.findWhere(players, { state: "dying"}));
}

function applyBoundaryDeath() {
    if(player.x < stageBoundary.left) player.state = "dying";
    if(player.x > stageBoundary.right) player.state = "dying";
}

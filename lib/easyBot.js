...
var gravity = require('../common/gravity');
...
function standingRoutine(player, bot) {
    ...
    if(between(player.x, stageBoundary.left, stageBoundary.center - 100)) {
        player.left();
        player.down();
        bot.attackHeight = _.random(-150, -400);
        return true;
    }
    if(between(player.x, stageBoundary.center + 100, stageBoundary.right)) {
        player.right();
        player.down();
